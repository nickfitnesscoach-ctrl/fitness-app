...
import { Check, AlertCircle, X, ChevronLeft, Flame, Drumstick, Droplets, Wheat, RefreshCcw, ChevronRight } from 'lucide-react';
...
export const BatchResultsModal: React.FC<BatchResultsModalProps> = ({
    ...
    const retryableErrorCount = photoQueue.filter(
        (p) => p.status === 'error' && p.error !== 'Отменено'
    ).length;

    const selectedItem = useMemo(
        () => photoQueue.find((p) => p.id === selectedId),
        [photoQueue, selectedId]
    );
    ...
    if (selectedId && selectedItem) {
        return (
            <ResultDetailView
                ...
                onRetry={
                    selectedItem.status === 'error' && selectedItem.error !== 'Отменено'
                        ? () => {
                            setSelectedId(null);
                    ...
                    {photoQueue.map((item) => (
                        ...
                        >
                            ...
                            <div className="w-20 h-20 shrink-0 rounded-xl overflow-hidden bg-gray-200 relative aspect-square">
                                ...
                                <div
                                    className={`absolute top-1 right-1 w-6 h-6 rounded-full flex items-center justify-center ${item.status === 'success'
                                        ? 'bg-green-500'
                                        : item.error === 'Отменено'
                                            ? 'bg-gray-400'
                                            : 'bg-red-500'
                                ...
                                {item.status === 'success' && item.result ? (
                                    <>
                                        <h3 className="font-bold text-gray-900 truncate pr-2">
                                            {item.result.recognized_items.map((i) => i.name).join(', ') ||
                                                'Еда'}
                                        </h3>
                                    ...
                                    <>
                                        <h3
                                            className={`font-bold ${item.error === 'Отменено' ? 'text-gray-500' : 'text-red-600'
                                                }`}
                                        >
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-[70] animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-lg sm:rounded-3xl rounded-t-3xl h-[98vh] sm:h-auto sm:max-h-[95vh] flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-300">
                ...
                <div className="overflow-y-auto flex-1">
                    ...
                    {photoItem.status === 'success' && result ? (
                        <div className="p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-4 gap-2 p-4 bg-gray-50 rounded-2xl border border-gray-100">
                                <div className="text-center border-r border-gray-200 last:border-0">
                                    <div className="text-xs text-gray-500 font-medium font-bold uppercase tracking-wider">Ккал</div>
                                    <div className="text-base font-bold text-orange-600">{Math.round(result.total_calories)}</div>
                                </div>
                                <div className="text-center border-r border-gray-200 last:border-0">
                                    <div className="text-xs text-gray-500 font-medium font-bold uppercase tracking-wider">Белки</div>
                                    <div className="text-base font-bold text-gray-900">{Math.round(result.total_protein)}</div>
                                </div>
                                <div className="text-center border-r border-gray-200 last:border-0">
                                    <div className="text-xs text-gray-500 font-medium font-bold uppercase tracking-wider">Жиры</div>
                                    <div className="text-base font-bold text-gray-900">{Math.round(result.total_fat)}</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-xs text-gray-500 font-medium font-bold uppercase tracking-wider">Угл.</div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-3">
                                    {result.recognized_items.map((r, idx) => renderItemCard(r, idx))}
                                </div>
                            </div>
                        </div>
                    ) : (
                        <div className="p-12 text-center flex flex-col items-center justify-center">
                            <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${photoItem.error === 'Отменено' ? 'bg-gray-100' : 'bg-red-50'
                                }`}>
                                <AlertCircle size={32} className={photoItem.error === 'Отменено' ? 'text-gray-400' : 'text-red-500'} />
                            </div>
                            <h3 className={`text-xl font-bold mb-2 ${photoItem.error === 'Отменено' ? 'text-gray-500' : 'text-red-600'
                                }`}>
                                {photoItem.error === 'Отменено' ? 'Отменено' : 'Ошибка загрузки'}
                            </h3>
                            <p className="text-gray-500 max-w-xs mb-8">
                                {photoItem.error || 'Попробуйте ещё раз'}
                            </p>
