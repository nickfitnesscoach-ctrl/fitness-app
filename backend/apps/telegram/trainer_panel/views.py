...
@extend_schema(
    ...
    parameters=[
        OpenApiParameter(name="limit", required=False, type=int, description="Сколько записей вернуть (default 200, max 1000)"),
        OpenApiParameter(name="offset", required=False, type=int, description="Смещение для пагинации"),
    ],
)
@api_view(["GET"])
...
def clients_list(request):
    ...
    try:
        ...
        return Response({"error": "ID must be a positive integer"}, status=status.HTTP_400_BAD_REQUEST)
...
def get_subscribers_api(request):
    ...
    stats = {
        "total": int(counts.get("free", 0)) + int(counts.get("monthly", 0)) + int(counts.get("yearly", 0)),
