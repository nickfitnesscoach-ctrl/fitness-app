...
## ⚠️ Типовые ошибки разработчика

| Ошибка | Почему опасно | Правильно |
|--------|---------------|-----------|
| Проверять успех по redirect | `return_url` вызывается до подтверждения банком | Ждать webhook или polling `/billing/me/` |
| Обновлять UI без refresh() | Показывает неактуальный статус | Всегда запрашивать `/billing/me/` |
| Хранить price на фронте | Клиент может подменить сумму | Только `plan_code`, цена из БД |
| Доверять frontend статусу | XSS/injection могут подделать | Бэкенд — источник истины |
...
│  [Платёж у пользователя]                                         │
│     │                                                            │
│     ▼ ❌ Webhook не дошёл (firewall, nginx, IP)                  │
│  [webhooks/yookassa]                                             │
│     │                                                            │
│     ▼ ❌ Celery worker упал / очередь переполнена                │
│  [Celery task]                                                   │
│     │                                                            │
